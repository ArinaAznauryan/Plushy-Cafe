...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class ProximitySelector : MonoBehaviour
    {
        ...
        [Tooltip("Assign screen position to the display selection.")]
        public Vector2 selectorPosition;

        [Tooltip("Assign radius to the display selection.")]
        public float radiusMultiplier;

        public int floatingOrder = 0;
        ...
        protected GameObject prevFocusObj;

        float radius, angle;
        PolarCord polar;
        KartesianCord kartesian = new KartesianCord(0f, 0f);
        ...
        {
            angle = 90f * floatingOrder;

            prevFocusObj = GameObject.FindWithTag("Ground");
        ...
        public GameObject GetObjectOnFocus()
        {
        ...
        public void CheckObjectOnFocus(Camera camera)
        {
            RaycastHit hit;
        ...
        public void UpdateObjectFocus(Camera camera)
        {
            CheckObjectOnFocus(camera);
        }

        protected virtual void FixedUpdate()
        {
            Camera localCamera = GameEventsManager.instance.player.movementController.camera;

            UpdateObjectFocus(localCamera);
        }
        ...
        protected virtual void Update()
        {
            ...
            GameObject ObjectOnFocus = GetObjectOnFocus();
            ...
            if (prevFocusObj != null && (ObjectOnFocus == null || 
                ObjectOnFocus.GetComponent<Usable>() == null || !ObjectOnFocus.GetComponent<Usable>().enabled)) {
                    RemoveAllUsablesFromDetectedList();
                    CheckTriggerExit(prevFocusObj);
                    currentUsable = null;
                }

            prevFocusObj = ObjectOnFocus;

            if (ObjectOnFocus?.GetComponent<Usable>()) CheckTriggerEnter(ObjectOnFocus);
        ...
        public virtual void UseCurrentSelection()
        {
            ...
            {
                currentUsable.OnUseUsable(useKey);
        ...
        public virtual void RemoveAllUsablesFromDetectedList()
        {
            usablesInRange = new List<Usable>();
        }
        ...
        public virtual void SetCurrentUsable(Usable usable)
        {
            ...
            if (usable != null)
            {
                ...
                currentUseMessage = DialogueManager.GetLocalizedText(string.IsNullOrEmpty(GetOverrideMessage()) ? defaultUseMessage : GetOverrideMessage());
            }
            else
            {
        ...
        protected string GetOverrideMessage() {
            switch (useKey) {
                case KeyCode.E: 
                    return currentUsable.overrideUseMessages.eMessage;
                case KeyCode.F:
                    return currentUsable.overrideUseMessages.fMessage;
                case KeyCode.R:
                    return currentUsable.overrideUseMessages.rMessage;
                default: break;
            }
            return null;
        }
        ...
        public virtual void OnGUI()
        {
            ...
            if (currentUsable != null)
            {
                radius = Vector3.Distance(new Vector3(0f, 0f, 0f), Camera.main.WorldToScreenPoint(currentUsable.transform.position));
                //Debug.Log("Distance: " + Camera.main.WorldToScreenPoint(currentUsable.transform.position));
                radius *= radiusMultiplier;
                polar = new PolarCord(radius, angle);
                kartesian = kartesian.PolarToKartesian2D(polar);

                GUI.skin = guiSkin;
                UnityGUITools.DrawText(new Rect(0, 0, Screen.width, Screen.height), currentHeading, guiStyle, textStyle, textStyleColor);
                UnityGUITools.DrawText(new Rect(kartesian.x, kartesian.y, Screen.width, Screen.height), currentUseMessage, guiStyle, textStyle, textStyleColor);
            }
        }
        ...
        //         RaycastHit hit;
        //         if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, maxSelectionDistance))
        //         {
        //             distance = Vector3.Distance(actorTransform.position, hit.collider.transform.position);
