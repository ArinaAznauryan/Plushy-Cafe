...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class Selector : MonoBehaviour, IEventSystemUser
    {
        ...
        protected virtual void Run3DRaycast()
        {
            ...
            //RaycastHit hit;
            //Ray ray = Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, maxSelectionDistance);
            lastRay = ray;
            ...
            else
            {
                ...
                {
                    Debug.Log("Are you fucking kidding me?: " + hit.collider.gameObject.name);
                    ...
                    if (hitUsable != null && hitUsable.enabled)
                    {
                        Debug.Log("IT HAS A USABLE, BIACH");
                        if (selection != hit.collider.gameObject)
                        {
                            Debug.Log("FUCKING SELECT IT ALREADY");
                            SetCurrentUsable(hitUsable);
                        }
                    }
                    else
                    {
                        Debug.Log("WHY TF WOULD YOU DESELECT IT");
                        DeselectTarget();
                    }
                }
                else
